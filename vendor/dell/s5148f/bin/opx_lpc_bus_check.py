...
resource='/dev/port'
write_value = 0xAF;
s51xx_cpld_scratch_reg = 0x102;
s51xx_smf_scratch_reg  = 0x202;
...
def cpld_lpc_bus_test():
    portio_reg_write (resource,s51xx_cpld_scratch_reg,write_value)
    read_value1 = portio_reg_read(resource,s51xx_cpld_scratch_reg)
    if (read_value1 != write_value):
        sytem_fault_notify()

# SMF LPC bus test
def smf_lpc_bus_test():
    portio_reg_write (resource,s51xx_smf_scratch_reg,write_value)
    read_value2 = portio_reg_read(resource,s51xx_smf_scratch_reg)
