...
# Get SMF version
r=0x`/usr/bin/portiocfg.py --get --offset 0x200 | sed 's/reg value //'`
echo "SMF-FPGA: $((r >> 4)).$((r & 0xf))" >> $FIRMWARE_VERSION_FILE

#Get SMF-MSS version
r=0x`/usr/bin/smfmbox.py --read 0x0002 | sed 's/00000000: //' | sed 's/ .*//'`
echo "SMF-MSS: $((r >> 6)).$(((r >> 4) & 0x3)).$((r & 0xf))" >> $FIRMWARE_VERSION_FILE
# Get CPLD1 version
